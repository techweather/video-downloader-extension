// Register context menus on install — must be done here, not top-level,
// to avoid duplicates when the service worker restarts.
...
// Self-contained function injected into the page to detect video embeds + page metadata.
// Must be fully self-contained — no references to outer service-worker scope.
function detectAndGetPageData() {
  ...
  document.querySelectorAll('iframe[src*="player.vimeo.com/video/"]').forEach((iframe) => {
    try {
      ...
      if (match) {
        ...
        if (processedIds.has('vimeo_' + vimeoId)) return;
        processedIds.add('vimeo_' + vimeoId);
        const title = findVideoTitle(iframe) || 'Vimeo Video ' + vimeoId;
        allVideos.push({ id: vimeoId, url: iframe.src.split('#')[0], title, platform: 'vimeo' });
      }
    } catch (e) {}
  });
  ...
  document.querySelectorAll('iframe[src*="youtube.com/embed/"], iframe[src*="youtube-nocookie.com/embed/"]').forEach((iframe) => {
    try {
      ...
      if (match) {
        ...
        if (processedIds.has('youtube_' + youtubeId)) return;
        processedIds.add('youtube_' + youtubeId);
        const title = findVideoTitle(iframe) || 'YouTube Video ' + youtubeId;
        allVideos.push({ id: youtubeId, url: 'https://www.youtube.com/watch?v=' + youtubeId, title, platform: 'youtube' });
      }
    } catch (e) {}
  });
  ...
  if (nextDataEl) {
    try {
      ...
      let m;
      while ((m = re.exec(nextDataEl.textContent)) !== null) muxIds.add(m[1]);
    } catch (e) {}
  }
  ...
  muxIds.forEach(playbackId => {
    ...
    allVideos.push({ id: playbackId, url: 'https://stream.mux.com/' + playbackId + '/high.mp4', title, platform: 'mux' });
  });

  // Page metadata
  let thumbnail = null;
  for (const selector of ['meta[property="og:image"]', 'meta[name="twitter:image"]', 'meta[itemprop="thumbnailUrl"]', 'link[rel="image_src"]']) {
  ...
  return {
    embeds: allVideos,
    pageData: { title: document.title, thumbnail, source: window.location.hostname, url: window.location.href }
  };
}

// Self-contained DOM scan injected into the page as a last resort.
// Returns results directly instead of using message passing.
function domScanForVideos() {
  ...
  if (nextDataScript) {
    try {
      for (const m of nextDataScript.textContent.matchAll(/"playbackId"\s*:\s*"([A-Za-z0-9]+)"/g)) muxIds.add(m[1]);
    } catch (e) {}
  }
  document.querySelectorAll('script:not([src])').forEach(script => {
    const text = script.textContent;
...
async function triggerCombinedVideoDownload(tab) {
  try {
    ...
    if (classified.supported) {
      ...
      const scan = scanResults[0].result;
    ...
    console.error('triggerCombinedVideoDownload error:', error);
  }
}
...
function sendToNativeApp(data, tabId) {
