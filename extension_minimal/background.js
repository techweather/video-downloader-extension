...
// 5. If not supported, fall back to DOM scan
            ...
            code: `
              (function() {
                ...
                try {
                  ...
                  videoElements.forEach((video, index) => {
                    if (video.src) {
                      ...
                      if (url && !processedUrls.has(url)) {
                        ...
                        if (!title) { const fn = getFilenameFromUrl(url); title = fn || ('Video ' + (videos.length + 1)); }
                    ...
                    ['data-video-src', 'data-mp4-src', 'data-src'].forEach(attr => {
                      ...
                      if (url) {
                        ...
                        if (fullUrl && !processedUrls.has(fullUrl)) {
                          ...
                          if (!title) { const fn = getFilenameFromUrl(fullUrl); title = fn || ('Video ' + (videos.length + 1)); }
                  ...
                  // 10. __NEXT_DATA__ and inline scripts for Mux playbackId
