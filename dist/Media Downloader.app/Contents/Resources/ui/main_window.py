...
class MainWindow(QMainWindow):
    ...
    def _create_settings_section(self, parent_layout):
        ...
        self.save_metadata_checkbox = QCheckBox("Save metadata files (thumbnails, descriptions, etc.)")
        self.save_metadata_checkbox.setChecked(self.settings.get('save_metadata', False))
        self.save_metadata_checkbox.toggled.connect(self.save_settings)
        self.save_metadata_checkbox.setStyleSheet("""
            QCheckBox { 
                color: #333;
                spacing: 8px;
            }
        ...
        organization_row.addWidget(self.save_metadata_checkbox)
    ...
    def add_download(self, data):
        ...
        self.download_queue.put({
            ...
            'save_metadata': self.settings.get('save_metadata', False)
        })
            ...
            if dialog.exec_():
                ...
                if selected:
                    ...
                    for video_info in selected:
                        if video_info['type'] == 'hls' or video_info['url'].endswith('.m3u8'):
                            # HLS stream - use yt-dlp
                            self.add_download({
                                'url': video_info['url'],
                                'title': video_info['original_title'],
                                'type': 'video',
                            ...
                            # Direct MP4 - use direct download
                            self.add_direct_video_download({
                                'url': video_info['url'],
